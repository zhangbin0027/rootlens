...
4. Read `src/rootlens/q1_5.py` `_check_build_oom_killed` — confirm only `re.search(r'oom[_-]kill', ...)`, no bare `Killed` or `Out of memory`
5. Read `src/rootlens/decision.py` `_intersect` — confirm body is only `return changed & error`
